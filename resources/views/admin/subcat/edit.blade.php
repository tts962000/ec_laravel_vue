...
@section('content')
    <div class="container">
        <form action="{{route('subcat.update',$subcat->id)}}" method="POST" enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="_method" value="PATCH">
            <div class="mb-3">
              <label for="exampleInputEmail1" class="form-label">Create Category</label>
              <input type="text" name="name" value="{{old('name',$subcat->name)}}" class="form-control"  aria-describedby="emailHelp">
            </div>
            <div class="mb-3">
               <p>Current Image=></p>

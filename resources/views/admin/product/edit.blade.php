                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="tag_id" id="tag_id">
                            @foreach ($tags as $tag)
                                <option value={{$tag->id}} @if ($tag->id==$product->tag_id) selected @endif>
                                    {{$tag->name}}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="integer" name="price" value="{{old('price',$product->price)}}">
                        @error('price')
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <img width="50" height="50" src="{{url('/images/'.$product->image)}}" alt="">
                        @error('image')
                        <div class="alert alert-danger">{{ $message }}</div>
                     @enderror
                    </div>
                    <div class="col-md-12 mb-3">
                        ...
                        <textarea name="description" type="text" id="" cols="20" rows="3">{{old('description',$product->description)}}</textarea>
                        @error('description')
                        <div class="alert alert-danger">{{ $message }}</div>
                     @enderror
                    </div>
                        <button type="submit" class="col-md-12 btn bg-success">Update Food Now</button>
                </div>
            </form>
        </div>
    </div>
...
@push('script')
    <script>
        ...
        filterSubcats(selectedCategory);
    </script>
@endpush
